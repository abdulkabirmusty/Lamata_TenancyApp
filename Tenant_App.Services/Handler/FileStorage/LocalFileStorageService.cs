...
namespace Tenant_App.Services.Handler.FileStorage
{
    public class LocalFileStorageService : IFileStorageService
    {
        ...
        where T : class
        {
            ...
            string extractedData = ExtractBase64Data(request.Data);

            var streamData = new MemoryStream(Convert.FromBase64String(extractedData));
            if (streamData.Length > 0)
            {
                string folder = typeof(T).Name;
                if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                ...
                {
                    FileType.Image => Path.Combine("Files", "Images", folder),
                    FileType.pdf => Path.Combine("Files", "Pdf", folder),
                    FileType.video => Path.Combine("Files", "Video", folder),
                    _ => Path.Combine("Files", "Others", folder),
                };
                string pathToSave = Path.Combine(Directory.GetCurrentDirectory(), folderName);
                Directory.CreateDirectory(pathToSave);
        ...
        public string ExtractBase64Data(string base64WithFormat)
        {
            return base64WithFormat.Split(',').Length == 2 ? base64WithFormat.Split(',')[1] : base64WithFormat;
        }
    }
}